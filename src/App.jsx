...
<div className="flex flex-wrap gap-2 mb-2">{PRE.map((p,i)=><button key={i} onClick={()=>onChange({...p.s})} className="px-3.5 py-2 text-xs sm:text-sm bg-gray-800 hover:bg-gray-700 rounded-full border border-gray-700 transition-colors">{p.n}</button>)}</div>
...
<div className="flex justify-center gap-2 pb-2 flex-wrap px-4">{Array.from({length:fCnt},(_,i)=><span key={i} className="text-sm px-3 py-1.5 rounded-full" style={{border:`1px solid ${FC[i]?.bg}60`,color:FC[i]?.bg}}>●{i===0?'你':`電腦${i}`}</span>)}</div>
...
<div><div className="text-base font-semibold text-gray-400 mb-1.5">職業</div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">{CLS.map(c=><button key={c.id} onClick={()=>setSelCls(c)}
className={`p-3 sm:p-4 rounded-lg border text-center transition-all ${selCls.id===c.id?'border-blue-500 bg-blue-500/10':'border-gray-700 bg-gray-800/50 hover:bg-gray-800'}`}>
<div className="text-2xl leading-none">{c.icon}</div><div className="text-sm text-gray-300 mt-1.5">{c.name}</div><div className="text-xs text-gray-500 mt-1 leading-tight">{c.d}</div></button>)}</div></div>
<div><div className="text-base font-semibold text-gray-400 mb-1.5">行動傾向</div>
<div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{BEH.map(b=><button key={b.id} onClick={()=>setSelBeh(b)}
className={`p-3 sm:p-3.5 rounded-lg border text-left transition-all ${selBeh.id===b.id?'border-green-500 bg-green-500/10':'border-gray-700 bg-gray-800/50 hover:bg-gray-800'}`}>
...
className={`px-5 py-3 rounded-lg border text-sm transition-all ${selTP===t.id?'border-amber-500 bg-amber-500/10 text-amber-300':'border-gray-700 bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{t.i} {t.n}</button>)}</div></div>
...
<div className="flex items-center gap-1 sm:gap-1.5">{[1,2,4,8].map(sp=><button key={sp} onClick={()=>setSpeed(sp)} className={`px-2.5 py-1.5 sm:px-3.5 sm:py-2 rounded-lg transition-colors text-xs sm:text-sm font-medium ${speed===sp?'bg-blue-600 text-white':'bg-gray-700 hover:bg-gray-600 text-gray-300'}`}>{sp}x</button>)}
<div className="relative">
<button onClick={()=>{setPaused(true);setPauseMenu(!pauseMenu)}} className="px-2.5 py-1.5 sm:px-3.5 sm:py-2 bg-gray-700 hover:bg-gray-600 rounded-lg flex items-center gap-1"><Menu size={16} className="sm:w-[18px] sm:h-[18px]"/></button>
</div></div></div>
...
<div><div className="text-sm sm:text-base font-semibold text-gray-400 mb-1">後代職業</div><div className="grid grid-cols-3 sm:grid-cols-6 gap-2">{CLS.map(c=><button key={c.id} onClick={()=>setSCls(c)}
className={`p-2.5 sm:p-3 rounded-lg border text-center transition-all ${sCls.id===c.id?'border-blue-500 bg-blue-500/10':'border-gray-700 bg-gray-800/50 hover:bg-gray-700'}`}><div className="text-xl">{c.icon}</div><div className="text-xs text-gray-500 mt-0.5">{c.name}</div></button>)}</div></div>
<div><div className="text-sm sm:text-base font-semibold text-gray-400 mb-1">行動傾向</div><div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{BEH.map(b=><button key={b.id} onClick={()=>setSBeh(b)}
className={`p-2.5 sm:p-3 rounded-lg border text-center transition-all ${sBeh.id===b.id?'border-green-500 bg-green-500/10':'border-gray-700 bg-gray-800/50 hover:bg-gray-700'}`}><span className="text-lg">{b.icon}</span><div className="text-xs text-gray-500 leading-tight mt-0.5">{b.name}</div></button>)}</div></div>
<div><div className="text-sm sm:text-base font-semibold text-gray-400 mb-1">目標優先</div><div className="flex gap-2 flex-wrap">{TPR.map(t=><button key={t.id} onClick={()=>setSTP(t.id)}
className={`px-4 py-2.5 rounded-lg border text-sm transition-colors ${sTP===t.id?'border-amber-500 bg-amber-500/10 text-amber-300':'border-gray-700 text-gray-400'}`}>{t.i} {t.n}</button>)}</div></div>
